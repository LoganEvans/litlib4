...
Mathematical Boundaries: Equipped with a Poisson bracket operation which must map smooth functions to smooth functions.
-/
...
Mathematical Boundaries: The Poisson bracket of any two constraints must be weakly zero, expressible as a linear combination of the constraints themselves via structure functions.
-/
class Eq2_2
...
Mathematical Boundaries: The Hamiltonian is a sum of a base Hamiltonian (which vanishes on the constraint surface if the system is totally constrained) and a linear combination of the constraints scaled by arbitrary multiplier functions.
-/
class Eq2_3
    {M : Type} (n : ℕ)
    (C : Fin n → M → ℝ)
...
Mathematical Boundaries: The gauge flow `α` must be a valid group action. The theorem strictly bounds the evolution to ensure that the prediction is gauge invariant on the constraint surface.
-/
class Theorem3_1
    {M : Type}
    (α : ℝ → M → M)
    (f T : M → ℝ)
    (F : ℝ → M → ℝ) : Prop where
  -- Flow Constraint: Explicitly demand α is a valid group action (flow) to prevent pathological non-flow maps.
...
Mathematical Boundaries: The map `α` must form a valid multi-dimensional abelian flow. The evaluation points must uniquely align the clock variables with the specified time parameters.
-/
class Theorem4_1
    {M : Type} (n : ℕ)
    (α : (Fin n → ℝ) → M → M)
...
Mathematical Boundaries: To ensure the clock variables properly parameterize the gauge orbit, the determinant of the Dirac matrix must be strictly non-zero, preventing topological collapse or coordinate singularity.
-/
class Eq5_6
    {M : Type} [PoissonSpace M] (n : ℕ)
    (C T : Fin n → M → ℝ)
