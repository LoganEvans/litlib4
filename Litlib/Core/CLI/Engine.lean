...
/-- Helper to convert a Lean Name (e.g. `CGD.Gravity.Foo`) into a relative filepath (`CGD/Gravity/Foo`). -/
  ...
  try
    for entry in ← dir.readDir do
      if ← entry.path.isDir then
        let nextPref := if pref == Name.anonymous then 
          Name.mkSimple entry.fileName 
        else 
          pref ++ Name.mkSimple entry.fileName
...
def unionNameSet (a b : NameSet) : NameSet :=
...
partial def collectDepsMemo (env : Environment) (n : Name) : DepM DepResult := do
  ...
  | some info =>
    ...
    if let some v := info.value? then
      consts := v.foldConsts consts (fun n acc => acc.insert n)

    for c in consts.toList do
      let childRes ← collectDepsMemo env c
      res := {
        hasSorry := res.hasSorry || childRes.hasSorry,
        litRefs := unionNameSet res.litRefs childRes.litRefs,
        localDecls := unionNameSet res.localDecls childRes.localDecls
      }
  | none => pure ()
...
-- ==========================================
-- Source Code & Fallback Formatting
-- ==========================================

/-- Deep crawler to robustly find the .lean source file anywhere in the project or dependencies. -/
...
/-- Extracts the exact typed text string from the `.lean` source file via the LSP's range mappings. -/
def extractSourceCode (env : Environment) (n : Name) : CoreM (Option String) := do
  ...
  if startLine < lines.size then
    ...
    -- Ensure docstrings are captured if the range missed them
...
def withPPOptions {α} (x : MetaM α) : MetaM α := do
  let opts := (← getOptions)
    ...
    |>.setNat `maxRecDepth 1000000
    |>.setNat `pp.maxDepth 1000000
    ...
    |>.setBool `pp.analyze.typeAscription false
...
def buildPrefixes (env : Environment) (rootModule : Name) : Array String := Id.run do
  ...
  for mod in env.header.moduleNames do
    ...
    if mStr.startsWith rootStr || mStr.startsWith "Litlib" then
      arr := arr.push s!"{mStr}."
  if !arr.contains s!"{rootStr}." then
    arr := arr.push s!"{rootStr}."
...
def cleanTypes (prefixes : Array String) (s : String) : String :=
  ...
  stripped.replace "Complex" "ℂ" 
          |>.replace "Real" "ℝ"
          |>.replace "failed to pretty print expression (use 'set_option pp.rawOnError true' for raw representation)" "<expression too large>"
...
def ppSignature (type : Expr) (isStructure : Bool) (prefixes : Array String) : MetaM String := withPPOptions do
  Meta.forallTelescope type fun fvars body => do
    ...
    if isStructure then return String.intercalate "\n" reqs.toList
      ...
      if reqs.isEmpty then return s!" : {bodyStr}"
      else return s!"{String.intercalate " " reqs.toList} :\n  {bodyStr}"

/-- Robust AST printer fallback formatted as valid Lean code syntax. -/
def ppDeclFallback (env : Environment) (n : Name) (prefixes : Array String) : MetaM String := withPPOptions do
  let docStr := match ← Lean.findDocString? env n with
    | some doc => s!"/--\n{doc.trimAscii.toString}\n-/\n"
    | none => ""

  let declStr ← match env.find? n with
    | some (ConstantInfo.thmInfo t) =>
      let sigStr ← ppSignature t.type false prefixes
      pure s!"theorem {n.getString!} {sigStr}"
    | some (ConstantInfo.defnInfo d) =>
      let sigStr ← ppSignature d.type false prefixes
      pure s!"def {n.getString!} {sigStr} := ..."
      ...
      if Lean.isStructure env n then
        ...
        | ctor :: _ =>
          if let some (ConstantInfo.ctorInfo c) := env.find? ctor then
            fieldsStr ← ppSignature c.type true prefixes
        | [] => pure ()
        ...
        for ctor in i.ctors do
          if let some (ConstantInfo.ctorInfo c) := env.find? ctor then
            let cTypeStr := cleanTypes prefixes (toString (← Meta.ppExpr c.type))
            let cName := match ctor with | .str _ s => s | _ => ctor.toString
            ctorStrs := ctorStrs.push s!"  | {cName} : {cTypeStr}"
        let ctorsBlock := String.intercalate "\n" ctorStrs.toList
...
/--
Parenthesis-aware token scanner to strip away proofs.
It accurately tracks `let`, `have`, and `obtain` keywords at the top-level 
so it doesn't accidentally truncate theorems that contain intermediate `:=` assignments in their types.
-/
def stripProofToSignature (s : String) : String := Id.run do
  ...
  while i < chars.size do
    ...
    if level == 0 && i + 1 < chars.size then
...
/-- Evaluates a declaration, extracts it from the source text or AST, and safely strips away proof bodies. -/
def processDecl (env : Environment) (declName : Name) (prefixes : Array String) (isSorry : Bool) : CoreM String := do
  ...
  let appendStatus (base : String) : String :=
    if isAxiom then base ++ " := axiom"
    else if isOpaque then base ++ " := opaque"
    else if isSorry then base ++ " := sorry"
    else base ++ " := ..."

  if let some src ← extractSourceCode env declName then
    if isDef || isStruct || isInduct then
      return src
    else
      let stripped := stripProofToSignature src
      return appendStatus stripped.trimAsciiEnd.toString
  else
    let ast ← Meta.MetaM.run' (ppDeclFallback env declName prefixes)
    if isDef || isStruct || isInduct then
      return ast
    else
      return appendStatus ast.trimAsciiEnd.toString

-- ==========================================
-- Extracted Presentation Data Models
-- ==========================================
...
/-- 
Evaluates the unified environment entirely inside CoreM to extract a flat, deduplicated Project-Wide Dependency Subgraph.
-/
def extractAllTheorems (env : Environment) : CoreM GlobalData := do
  ...
  let envTagged := env.constants.fold (fun acc declName _ =>
  ...
  for (declName, desc) in envTagged do
    ...
    for ref in depRes.litRefs.toList do
      if let some data := litlibExt.find? env ref then
        ...
        if !globalLitRefs.contains ref then
          globalLitRefs := globalLitRefs.insert ref data

    theorems := theorems.push {
      name := declName,
      desc := desc,
      hasSorry := depRes.hasSorry,
      litRefs := thmLitRefs
    }
    ...
    if let some info := env.find? declName then
      ...
      for c in consts.toList do
        let isExt := match env.getModuleIdxFor? c with
          | some idx => 
            let modName := env.header.moduleNames[idx.toNat]!
            Name.isPrefixOf `Mathlib modName || Name.isPrefixOf `Lean modName || Name.isPrefixOf `Init modName || Name.isPrefixOf `Std modName || Name.isPrefixOf `Lake modName
          | none => false
  ...
  theorems := theorems.qsort (fun a b => a.name.toString < b.name.toString)
  
  let mut litRefsArr := #[]
  for (n, d) in globalLitRefs do
    litRefsArr := litRefsArr.push { name := n, data := d }

  return { theorems := theorems, litRefs := litRefsArr, summaryItems := summaryItems }
