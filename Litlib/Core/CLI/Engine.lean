...
/-- 
Performs simple matching on a string. 
Supported syntax:
- `^text` (Starts with)
- `text$` (Ends with)
- `^text$` (Exact match)
...
def globMatch (pat target : String) : Bool :=
  ...
  if p == "" then false
    ...
    target == myDropRight (myDropLeft p 1) 1
  else if p.startsWith "^" then
    target.startsWith (myDropLeft p 1)
  else if p.endsWith "$" then
    target.endsWith (myDropRight p 1)
  else
    (target.splitOn p |>.length) > 1
...
def discoverModules (rootModule : Name) : IO (Array Name) := do
  ...
  let mut targetDir : Option System.FilePath := none
  for d in candidates do
    if ← d.isDir then
      if ← (d / s!"{rootModule}.olean").pathExists then
        targetDir := some d
        break
      else if ← (d / rootModule.toString).isDir then
        targetDir := some d
        break

  if let some libDir := targetDir then
    let allMods ← findOleans libDir Name.anonymous
    let filtered := allMods.filter (fun m => Name.isPrefixOf rootModule m)
    if filtered.isEmpty then return #[rootModule] else return filtered
  else
    return #[rootModule]
...
def extractAllTheorems (env : Environment) (ctx : CliContext) : CoreM GlobalData := do
  ...
  for (declName, desc) in thmKeys do
    ...
    if let some info := env.find? declName then
      ...
      for c in consts.toList do
        ...
        if !isExt && c != declName && findLitRef env c == none && !isAutoGenerated env c then
          if !renderedNames.contains c then
            renderedNames := renderedNames.insert c
            let (cRes, nextState) := (collectDepsMemo env c).run depState
            depState := nextState
            ...
            summaryItems := summaryItems.push { moduleName := modName, declName := c, codeStr := code, type := "localDef", desc := "" }

  -- 2. Filter the gathered items based on the CLI context
  let mut filteredTheorems := #[]
  if ctx.targetTheorems then
    filteredTheorems := theorems.filter (fun t => matchesAnyGlob ctx.theoremGlobs t.name.toString)
  
  -- Force unlinked references to load if requested specifically
  if ctx.targetReferences then
    let consts := env.constants.fold (fun acc n _ => acc.push n) #[]
    for n in consts do
      if matchesAnyGlob ctx.referenceGlobs n.toString then
        if let some data := litlibExt.find? env n then
          if !globalLitRefs.contains n then
            globalLitRefs := globalLitRefs.insert n data
  ...
  let mut litRefsArr := #[]
  for (n, d) in globalLitRefs do
    if ctx.targetReferences then
      if matchesAnyGlob ctx.referenceGlobs n.toString then
        litRefsArr := litRefsArr.push { name := n, data := d }
    else if ctx.targetTheorems then
      -- If only theorems targeted, include references attached to those filtered theorems
      if filteredTheorems.any (fun t => t.litRefs.any (fun r => r.name == n)) then
        litRefsArr := litRefsArr.push { name := n, data := d }

  -- Filter summary items to match the targeted entities
  let finalSummary := summaryItems.filter fun item =>
    if item.type == "theorem" then matchesAnyGlob ctx.theoremGlobs item.declName.toString && ctx.targetTheorems
    else if item.type == "litRef" then matchesAnyGlob ctx.referenceGlobs item.declName.toString && ctx.targetReferences
    else true -- Keep localDefs if the parent theorem was kept (we could do deep-filtering, but keeping them is fine for context)
