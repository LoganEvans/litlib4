...
class LeftHaarMeasure (G : Type*) [TopologicalSpace G] [Group G] [MeasurableSpace G]
    [BorelSpace G] (μ : MeasureTheory.Measure G) : Prop where
  is_nonzero : μ ≠ 0
  is_locally_finite : MeasureTheory.IsLocallyFiniteMeasure μ
  is_left_invariant : ∀ (g : G) (E : Set G), MeasurableSet E → μ ((fun x ↦ g * x) '' E) = μ E

Litlib.equation "hall2003lie" eq "Unimodular" page "121" kind "definition"
class Unimodular (G : Type*) [TopologicalSpace G] [Group G] [MeasurableSpace G]
    [BorelSpace G] : Prop where
