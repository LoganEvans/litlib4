...
class Theorem_Eq2_2c_RicciFlat
  ...
  /-- Spinor Metric Antisymmetry: The fundamental spinor metrics must be antisymmetric to properly
  raise and lower $SL(2, \mathbb{C})$ indices. -/
...
Litlib.equation "capovilla1991pure"
  eq "Page 61, Theorem C"
  page "61"
  kind "Theorem"
class Theorem_Eq2_2c_EinsteinSpace
    (Spacetime : Type*) [TopologicalSpace Spacetime]
    (theta : Spacetime → Fin 4 → Fin 2 → Fin 2 → ℂ)
    (g : Spacetime → Fin 4 → Fin 4 → ℂ)
    (eps2_down : Fin 2 → Fin 2 → ℂ)
  ...
  /-- Spinor Metric Antisymmetry: The fundamental spinor metrics must be antisymmetric. -/
  heps2_anti : ∀ A B, eps2_down A B = - eps2_down B A
  heps2_bar_anti : ∀ A' B', eps2_bar_down A' B' = - eps2_bar_down B' A'
  heps2_right_anti : ∀ A' B', eps2_right A' B' = - eps2_right B' A'
  heps2_up_anti : ∀ A B, eps2_up A B = - eps2_up B A
  ...
  eq2_2c_implies_einstein_space :
    -- (1) Tetrad relation (Eq 2.3)
    (∀ x μ ν A B, Sigma x μ ν A B = (1/2 : ℂ) * sumFin2 fun A' ↦ sumFin2 fun B' ↦
      eps2_right A' B' * (theta x μ A A' * theta x ν B B' - theta x ν A A' * theta x μ B B')) →
    -- (2) Torsion-free connection (Eq 2.2b)
    (∀ x μ ν ρ A B,
      let omega_up := fun lam A' C' ↦ sumFin2 fun E ↦ eps2_up A' E * omega x lam E C';
      let term := fun m n r ↦ dSigma x m n r A B +
        sumFin2 (fun C ↦ omega_up m A C * Sigma x n r B C + omega_up m B C * Sigma x n r A C);
      term μ ν ρ + term ν ρ μ + term ρ μ ν - term ν μ ρ - term μ ρ ν - term ρ ν μ = 0) →
    -- (3) Curvature expansion with cosmological constant (Eq 2.2c modified by Lambda / 6)
    (∀ x μ ν A B, R x μ ν A B =
      (sumFin2 fun C ↦ sumFin2 fun D ↦ Psi x A B C D * Sigma x μ ν C D) +
      (Lambda / 6) * Sigma x μ ν A B) →
    -- Conclusion: Metric is an Einstein space with R_μν = Λ g_μν
...
-/
@[litlib_track "Derived volume element identity from Capovilla constraints"]
theorem volume_element_identity_derived
    (Spacetime : Type*) [TopologicalSpace Spacetime]
    (sqrt_g : Spacetime → ℂ)
    (mu : Spacetime → ℂ)
    (eta : Spacetime → ℂ)
    (Psi_3x3 : Spacetime → Matrix (Fin 3) (Fin 3) ℂ)
    (M_3x3 : Spacetime → Matrix (Fin 3) (Fin 3) ℂ)
    [inst : Theorem_Volume_Element_Identity Spacetime sqrt_g mu eta Psi_3x3 M_3x3] (x : Spacetime) :
    (sqrt_g x)^2 = (mu x)^2 := by
  have hg : sqrt_g x ≠ 0 := inst.h_sqrt_g_nondeg x
  have hm : mu x ≠ 0 := inst.h_mu_nondeg x
  have hM : Matrix.det (M_3x3 x) ≠ 0 := inst.h_M_nondeg x
  have hP : Matrix.det (Psi_3x3 x) ≠ 0 := inst.h_Psi_nondeg x

  have eq16 := inst.eq2_16_holds x
  have eq21 := inst.eq2_21_holds x
  have eq11 := inst.eq2_11_det_holds x

  have h1 : eta x * (sqrt_g x * Matrix.det (Psi_3x3 x)) = 1 := by
    calc
      eta x * (sqrt_g x * Matrix.det (Psi_3x3 x))
        = (sqrt_g x * Matrix.det (Psi_3x3 x))⁻¹ * (sqrt_g x * Matrix.det (Psi_3x3 x)) := by rw [eq21]
      _ = 1 := inv_mul_cancel₀ (mul_ne_zero hg hP)

  have h2 : (eta x)^2 * (sqrt_g x)^2 * (Matrix.det (Psi_3x3 x))^2 = 1 := by
    calc
      (eta x)^2 * (sqrt_g x)^2 * (Matrix.det (Psi_3x3 x))^2
        = (eta x * (sqrt_g x * Matrix.det (Psi_3x3 x)))^2 := by ring
      _ = 1^2 := by rw [h1]
      _ = 1 := by ring

  have h3 : (eta x)^2 * Matrix.det (M_3x3 x) = mu x := by
    calc
      (eta x)^2 * Matrix.det (M_3x3 x)
        = (mu x / Matrix.det (M_3x3 x)) * Matrix.det (M_3x3 x) := by rw [eq16]
      _ = mu x := div_mul_cancel₀ _ hM

  have eq11_mod : (Matrix.det (Psi_3x3 x))^2 * (mu x)^3 = Matrix.det (M_3x3 x) := by
    have h_inv : (mu x)⁻¹ * mu x = 1 := inv_mul_cancel₀ hm
    calc
      (Matrix.det (Psi_3x3 x))^2 * (mu x)^3
        = ((mu x)⁻¹ * (mu x)⁻¹ * (mu x)⁻¹ * Matrix.det (M_3x3 x)) * (mu x)^3 := by rw [eq11]
      _ = Matrix.det (M_3x3 x) * ((mu x)⁻¹ * mu x)^3 := by ring
      _ = Matrix.det (M_3x3 x) * 1^3 := by rw [h_inv]
      _ = Matrix.det (M_3x3 x) := by ring

  have h9 : (sqrt_g x)^2 * mu x = (mu x)^3 := by
    calc
      (sqrt_g x)^2 * mu x
        = (sqrt_g x)^2 * ((eta x)^2 * Matrix.det (M_3x3 x)) := by rw [← h3]
      _ = (sqrt_g x)^2 * ((eta x)^2 * ((Matrix.det (Psi_3x3 x))^2 * (mu x)^3)) := by rw [← eq11_mod]
      _ = ((eta x)^2 * (sqrt_g x)^2 * (Matrix.det (Psi_3x3 x))^2) * (mu x)^3 := by ring
      _ = 1 * (mu x)^3 := by rw [h2]
      _ = (mu x)^3 := by ring

  have h10 : (sqrt_g x)^2 * mu x = (mu x)^2 * mu x := by
    calc
      (sqrt_g x)^2 * mu x = (mu x)^3 := h9
      _ = (mu x)^2 * mu x := by ring

  exact mul_right_cancel₀ hm h10
