...
class Eq3 where
  plebanskiTetradReconstruction :
    ∀ (e0 : Fin 4 → ℂ)
      ...
      (_hEps3 : eps3 0 1 2 = 1 ∧ ∀ i j k, eps3 i j k = -eps3 j i k ∧ eps3 i j k = -eps3 i k j ∧ eps3 i j k = -eps3 k j i)
      (eps4 : Fin 4 → Fin 4 → Fin 4 → Fin 4 → ℂ)
      (_hEps4 : eps4 0 1 2 3 = 1 ∧ ∀ a b c d, eps4 a b c d = -eps4 b a c d ∧ eps4 a b c d = -eps4 a c b d ∧ eps4 a b c d = -eps4 a b d c),
...
class Eq7 where
  bianchiTraceIdentity :
    ∀ (sigma : Fin 3 → Fin 4 → Fin 4 → ℂ)
      ...
      (eps3 : Fin 3 → Fin 3 → Fin 3 → ℂ)
      (_hSigmaAntisymm : ∀ i μ ν, sigma i μ ν = -sigma i ν μ)
      (_hSigmaSigma1 : ∀ i j, ∑ μ : Fin 4, ∑ ν : Fin 4, sigma i μ ν * sigma j μ ν = 4 * if i = j then (1:ℂ) else 0)
      (_hSigmaSigma2 : ∀ i j μ ρ, ∑ ν : Fin 4, sigma i μ ν * sigma j ν ρ = 
        -(if i = j then (1:ℂ) else 0) * (if μ = ρ then (1:ℂ) else 0) + ∑ k : Fin 3, eps3 i j k * sigma k μ ρ)
      (_hVSymm : ∀ i j ρ, v i j ρ = v j i ρ)
      (_hEps3Antisymm : ∀ i j k, eps3 i j k = -eps3 j i k ∧ eps3 i j k = -eps3 i k j),
...
class Eq11 where
  pureConnectionMatrix :
    ∀ (M Minv X : Matrix (Fin 3) (Fin 3) ℂ)
      (_hMSymm : ∀ i j, M i j = M j i)
      (_hMinvSymm : ∀ i j, Minv i j = Minv j i)
      (_hInv : Minv * M = 1)
      (_hEq : Minv * X * Minv = 1),
      X = M * M
...
class UnimodularCDJ where
  cdjImpliesConstantVolume :
    ∀ (SpacetimePoint : Type*)
      ...
      (_hLambdaNz : Λ ≠ 0)
      (_h_metric_zero : urbantkeMetric (fun _ _ _ => 0) = 0)
      (_hCdjConstraint : ∀ x, 
