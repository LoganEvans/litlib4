...
class RelativeInvariantGL4 (W : Type*) [AddCommGroup W] [Module ℂ W] where
  /--
  Predicate to restrict functions to polynomial forms.
  Weyl's First Fundamental Theorem strictly requires the invariant to be a polynomial.
  By making this a class field rather than a global opaque/axiom, we close the
  mathematical loophole that would allow transcendental or fractional functions
  to falsely evaluate to the bilinear form, while strictly satisfying Lean 4
  linters without needing `set_option`.
  -/
  isPolynomialFunction : ((Fin 4 → Fin 4 → W) → ℂ) → Prop
